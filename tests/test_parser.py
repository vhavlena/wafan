...
class TestParseRxRules:
    ...
    def test_rule_100_parsed(self, rx_rules):
        rule = next((r for r in rx_rules if r.rule_id == "100"), None)
        assert rule is not None

    def test_rule_100_variable(self, rx_rules):
        rule = next(r for r in rx_rules if r.rule_id == "100")
        assert any(v.name == "ARGS" for v in rule.variables)

    def test_rule_100_pattern(self, rx_rules):
        rule = next(r for r in rx_rules if r.rule_id == "100")
        assert "foo" in rule.operator_argument

    def test_negated_flag_false_for_rx(self, rx_rules):
        rule = next(r for r in rx_rules if r.rule_id == "100")
        assert rule.negated is False

    def test_negated_rule_absent_in_fixture(self):
